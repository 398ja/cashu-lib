...
public class BDHKEUtilsValidationTest {
  ...
  public void hashToCurveStringNullOrEmpty() {
    IllegalArgumentException nullEx =
        assertThrows(IllegalArgumentException.class, () -> BDHKEUtils.hashToCurve((String) null));
    assertEquals("secret must not be null or empty", nullEx.getMessage());

    IllegalArgumentException emptyEx =
        assertThrows(IllegalArgumentException.class, () -> BDHKEUtils.hashToCurve(""));
  ...
  public void hashToCurveBytesNullOrEmpty() {
    IllegalArgumentException nullEx =
        assertThrows(IllegalArgumentException.class, () -> BDHKEUtils.hashToCurve((byte[]) null));
    assertEquals("secret must not be null or empty", nullEx.getMessage());

    IllegalArgumentException emptyEx =
        assertThrows(IllegalArgumentException.class, () -> BDHKEUtils.hashToCurve(new byte[0]));
    assertEquals("secret must not be null or empty", emptyEx.getMessage());
  }
}
