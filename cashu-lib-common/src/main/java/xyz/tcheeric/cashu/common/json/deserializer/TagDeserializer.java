...
public class TagDeserializer extends JsonDeserializer<WellKnownSecret.Tag> {
  @Override
  public WellKnownSecret.Tag deserialize(JsonParser p, DeserializationContext ctxt)
    ...
    switch (P2PKSecret.P2PKTag.valueOf(key.toLowerCase())) {
      case P2PKSecret.P2PKTag.sigflag ->
          tag.addValue(P2PKSecret.SignatureFlag.valueOf(node.get(1).textValue()));
      case P2PKSecret.P2PKTag.pubkeys, P2PKSecret.P2PKTag.refund -> {
        for (int i = 1; i < node.size(); i++) {
          tag.addValue(node.get(i).textValue());
        }
      }
      case P2PKSecret.P2PKTag.n_sigs, P2PKSecret.P2PKTag.locktime ->
