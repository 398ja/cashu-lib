...
public interface Token {
  ...
  class TokenUtil {
    ...
    static String serialize(@NonNull String cborToken, Version version, boolean clickable) {
      return serialize(
          cborToken.replaceAll("\\s+", "").getBytes(StandardCharsets.UTF_8), version, clickable);
    }
    ...
    static String serialize(
        byte @NonNull [] cborToken, @NonNull String prefix, Version version, boolean clickable) {
      String strSerializedToken =
          prefix + version.getCode() + Base64.getUrlEncoder().encodeToString(cborToken);
      if (clickable) {
