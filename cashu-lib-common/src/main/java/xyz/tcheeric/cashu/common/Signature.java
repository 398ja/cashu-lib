...
public class Signature extends CryptoElement {
  ...
  public static Signature sign(@NonNull String message, @NonNull PrivateKey privateKey)
      throws Exception {
    byte[] signature =
        Schnorr.sign(message.getBytes(StandardCharsets.UTF_8), privateKey.getBytes());
    return Signature.fromBytes(signature);
  }

  public static boolean verify(
      @NonNull String message, @NonNull PublicKey publicKey, @NonNull Signature signature)
      throws Exception {
    byte[] pubKeyBytes = publicKey.getBytes();
    byte[] xOnlyPublicKey =
        pubKeyBytes.length == 33 ? Arrays.copyOfRange(pubKeyBytes, 1, 33) : pubKeyBytes;
    return Schnorr.verify(
        message.getBytes(StandardCharsets.UTF_8), xOnlyPublicKey, signature.getBytes());
  }
