...
/**
 * Compliance checks for NUT-02 keyset identifier derivation.
 */
class NUT02Tests {
    ...
    /**
     * Ensures NUT-02 example keyset hashes to id 00456a94ab4e1c46.
     */
    @Test
    void shouldDeriveExpectedKeysetIdForShortVector() throws Exception {
        // Arrange
        String json = """
                {
        ...
        Keys keys = OBJECT_MAPPER.readValue(json, Keys.class);

        // Act
        String keysetId = KeySetDerivation.getId(toByteMap(keys));
    ...
    /**
     * Ensures large NUT-02 vectors hash to the expected id 000f01df73ea149a.
     */
    @Test
    void shouldDeriveExpectedKeysetIdForLargeVector() throws Exception {
        // Arrange
        String json = """
                {
    ...
    private static Map<BigInteger, byte[]> toByteMap(Keys keys) {
        Map<BigInteger, byte[]> byteMap = new HashMap<>();
        for (Map.Entry<BigInteger, PublicKey> entry : keys.getValues().entrySet()) {
            byteMap.put(entry.getKey(), Hex.decode(entry.getValue().toString()));
        }
        return byteMap;
    }
}
