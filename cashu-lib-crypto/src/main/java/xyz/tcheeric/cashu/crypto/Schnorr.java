...
public class Schnorr {
  ...
  /**
   * Create a BIP-340 Schnorr signature for the provided message using the given secret key.
   *
   * <p>The implementation follows the algorithm defined in BIP-340 and operates on the secp256k1
   * elliptic curve. The message and secret key must each be exactly 32 bytes. The secret key is
   * interpreted as an integer in the range {@code [1, n-1]} where {@code n} is the curve order. The
   * produced signature is 64 bytes long and is verified internally before being returned.
   *
   * @param msg 32-byte message to sign
   * @param secKey 32-byte secp256k1 secret key
   * @return 64-byte Schnorr signature
   * @throws Exception if the message or key have invalid sizes, the secret key is out of range,
   *     nonce generation fails, or the resulting signature does not verify
   */
  public static byte[] sign(byte[] msg, byte[] secKey) throws Exception {
    ...
    byte[] t =
        Utils.xor(Utils.bytesFromBigInteger(secKey0), Point.taggedHash("BIP0340/aux", auxRand));
    ...
    BigInteger e =
        Utils.bigIntFromBytes(Point.taggedHash("BIP0340/challenge", buf)).mod(Point.getn());
    ...
    System.arraycopy(
        Utils.bytesFromBigInteger(kes),
        0,
        sig,
        R.toBytes().length,
        Utils.bytesFromBigInteger(kes).length);
   ...
   *
   * <p>All inputs are expected to be fixed-size byte arrays: the message and public key must both
   * be 32 bytes, while the signature must be 64 bytes consisting of the 32-byte X coordinate and
   * 32-byte scalar {@code s}. The method validates that the signature is consistent with the
   * provided data under the secp256k1 curve using the challenge computation described in BIP-340.
   *
   * @param msg 32-byte message that was signed
   * @param pubkey 32-byte x-only public key
   * @param sig 64-byte Schnorr signature
   * @return {@code true} if the signature is valid according to BIP-340, {@code false} otherwise
  ...
  /**
   * Generate a cryptographically-secure random private key for the secp256k1 curve.
   *
   * <p>The key is generated using the BouncyCastle provider and {@link
   * SecureRandom#getInstanceStrong()}. The returned byte array is 32 bytes long and suitable for
   * use with Schnorr signatures.
   *
   ...
   *
   * <p>The secret key must be a 32-byte scalar in the range {@code [1, n-1]}. The returned public
   * key is the 32-byte X coordinate of the point {@code secKey * G} on the secp256k1 curve, as
   * required by BIP-340.
   *
   * @param secKey 32-byte secp256k1 secret key
